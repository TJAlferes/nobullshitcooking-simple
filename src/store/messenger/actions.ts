...
export const messengerGetOnline = (online: IUser[]) =>
  ({type: MESSENGER_GET_ONLINE, online});

export const messengerShowOnline = (user: IUser) =>
  ({type: MESSENGER_SHOW_ONLINE, user});

export const messengerShowOffline = (user: IUser) =>
  ({type: MESSENGER_SHOW_OFFLINE, user});

export const messengerChangeChannel = (channel: string) =>
  ({type: MESSENGER_CHANGE_CHANNEL, channel});

export const messengerChangedChannel = (users: IUser[], channel: string) =>
  ({type: MESSENGER_CHANGED_CHANNEL, users, channel});

export const messengerRejoinedChannel = (users: IUser[], channel: string) =>
  ({type: MESSENGER_REJOINED_CHANNEL, users, channel});

export const messengerJoinedUser = (user: IUser) => {
  const ts = `${(new Date).toLocaleTimeString()}`;
  return {type: MESSENGER_JOINED_USER, user, ts};
};

export const messengerLeftUser = (user: IUser) => {
  const ts = `${(new Date).toLocaleTimeString()}`;
  return {type: MESSENGER_LEFT_USER, user, ts};
};

export const messengerSendMessage = (text: string) =>
  ({type: MESSENGER_SEND_MESSAGE, text});

export const messengerReceivedMessage = (
  message: IMessageBeforeClientTimestamp
) => {
  const ts = `${(new Date).toLocaleTimeString()}`;
  return {type: MESSENGER_RECEIVED_MESSAGE, message: {...message, ts}};
};

export const messengerSendWhisper = (text: string, to: string) =>
  ({type: MESSENGER_SEND_WHISPER, text, to});

export const messengerReceivedWhisper = (
  whisper: IWhisperBeforeClientTimestamp
) => {
  const ts = `${(new Date).toLocaleTimeString()}`;
  return {type: MESSENGER_RECEIVED_WHISPER, whisper: {...whisper, ts}};
};

export const messengerFailedWhisper = (feedback: string) => {
  const ts = `${(new Date).toLocaleTimeString()}`;
  return {type: MESSENGER_FAILED_WHISPER, feedback, ts};
};